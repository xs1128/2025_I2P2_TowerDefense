...
Plane::Plane()
    : Sprite("play/plane.png", -100,
             Engine::GameEngine::GetInstance().GetScreenHeight() / 2.0),
      stage(0), timeTicks(0)
{
    for (int i = 1; i <= 10; ++i) {
        bmps.push_back(Engine::Resources::GetInstance().GetBitmap(
            "play/light-" + std::to_string(i) + ".png"));
    }
    shockwave =
        Engine::Resources::GetInstance().GetBitmap("play/shockwave.png");
...
void Plane::Update(float deltaTime)
{
    ...
    case 0:
        ...
        if (!Engine::Collider::IsRectOverlap(
                ...
                Engine::Point(-100, 0), PlayScene::GetClientSize())) {
    ...
    case 1:
        ...
        scaleExp =
            ((timeSpanLight + timeSpanShockwave - timeTicks) * log2(minScale) +
             timeTicks * log2(maxScale)) /
            (timeSpanLight + timeSpanShockwave);
...
void Plane::Draw() const
{
    ...
    if (stage == 1 || stage == 2) {
        if (phase > 7 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 7].get(),
                al_map_rgba(0, 0, 255, 255 - phaseRatio * 64),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
        if (phase > 6 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 6].get(),
                al_map_rgba(0, 0, 255, 191 - phaseRatio * 64),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
        if (phase > 5 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 5].get(),
                al_map_rgba(0, 255, 0, 127 - phaseRatio * 64),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
        if (phase > 4 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 4].get(),
                al_map_rgba(255, 255, 255, phaseRatio * 63),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
        if (phase > 3 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 3].get(),
                al_map_rgba(255, 0, 0, 63 - phaseRatio * 63),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
        if (phase > 2 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 2].get(),
                al_map_rgba(255, 0, 0, 127 - phaseRatio * 64),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
        if (phase > 1 && phase < bmps.size()) {
            al_draw_tinted_scaled_rotated_bitmap(
                bmps[phase - 1].get(),
                al_map_rgba(0, 255, 0, 191 - phaseRatio * 64),
                Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
                Position.x, Position.y, Size.x / GetBitmapWidth(),
                Size.y / GetBitmapHeight(), Rotation, 0);
        }
    ...
    if (stage == 1)
        al_draw_tinted_scaled_rotated_bitmap(
            bmp.get(), al_map_rgba(255, 255, 255, 255 - phaseRatio * 64),
            Anchor.x * GetBitmapWidth(), Anchor.y * GetBitmapHeight(),
            Position.x, Position.y, Size.x / GetBitmapWidth(),
            Size.y / GetBitmapHeight(), Rotation, 0);
    else {
