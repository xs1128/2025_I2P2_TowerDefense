...
// DONE HACKATHON-4 (3/3): When the cheat code is entered, a plane should be
// spawned and added to the scene. DONE HACKATHON-5 (1/4): There's a bug in this
// file, which crashes the game when you win. Try to find it. DONE HACKATHON-5
// (2/4): The "LIFE" label are not updated when you lose a life. Try to fix it.
...
const std::vector<int> PlayScene::code = {
    ...
    ALLEGRO_KEY_LEFT, ALLEGRO_KEY_RIGHT,    ALLEGRO_KEY_B,
...
void PlayScene::OnKeyDown(int keyCode)
{
    ...
    if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        // is same size check if the content is the same by traversing the list
        bool flag = (keyStrokes.size() == code.size());
        if (flag) {
            ...
            if (flag) {
                Engine::LOG(Engine::INFO)
                    << "Cheat code triggered, spawning plane!";
                keyStrokes.clear();
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance()
{
    ...
    while (!que.empty()) {
        ...
        // Current dist from end (Prefix sum concept)
        int d = map[p.y][p.x];

        // Set 4 conditions for tile dirt to be valid
        if (p.x > 0 && mapState[p.y][p.x - 1] == TILE_DIRT &&
            map[p.y][p.x - 1] == -1) {
            map[p.y][p.x - 1] = d + 1;
            que.push(Engine::Point(p.x - 1, p.y));
        }
        if (p.x < MapWidth - 1 && mapState[p.y][p.x + 1] == TILE_DIRT &&
            map[p.y][p.x + 1] == -1) {
            map[p.y][p.x + 1] = d + 1;
            que.push(Engine::Point(p.x + 1, p.y));
        }
        if (p.y > 0 && mapState[p.y - 1][p.x] == TILE_DIRT &&
            map[p.y - 1][p.x] == -1) {
            map[p.y - 1][p.x] = d + 1;
            que.push(Engine::Point(p.x, p.y - 1));
        }
        if (p.y < MapHeight - 1 && mapState[p.y + 1][p.x] == TILE_DIRT &&
            map[p.y + 1][p.x] == -1) {
            map[p.y + 1][p.x] = d + 1;
            que.push(Engine::Point(p.x, p.y + 1));
        }
